...
export default function HomePage() {
  ...
  const handleNext = () => {
    if (selectedRole === null || jobDesc.trim() === "") {
      alert("Please select a role and enter job description!");
      return;
    }
    const role = roles[selectedRole].title;
    // Clear any previous session-selected files when starting fresh
    sessionStorage.removeItem("selectedFiles");
    sessionStorage.setItem("personaRole", role);
    sessionStorage.setItem("personaJob", jobDesc);
    router.push(`/upload?role=${encodeURIComponent(role)}&job=${encodeURIComponent(jobDesc)}`);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 flex flex-col items-center px-6 py-12">
      ...
      <div className="flex items-center mb-10">
        {[1, 2, 3].map((page, index) => (
          <div key={page} className="flex items-center">
            <div className={`w-10 h-10 flex items-center justify-center rounded-full font-semibold z-10 ${page === 1 ? "bg-red-600 text-white" : "bg-gray-300 text-gray-700"}`}>
              {page}
            </div>
            {index < 2 && <div className="w-16 h-1 mx-2 bg-gray-300"></div>}
          </div>
        ))}
      </div>
      ...
      <div className="text-center max-w-2xl mb-12">
        <h1 className="text-4xl font-extrabold text-red-600 tracking-tight">Adobe Document Intelligence</h1>
        <p className="text-gray-600 mt-4 text-lg">Select your persona and describe the task.</p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {roles.map((role, index) => (
            <div
              ...
              className={`rounded-2xl p-6 text-center transition cursor-pointer
                ${selectedRole === index ? "border-2 border-red-600 shadow-lg" : "border border-gray-200 shadow-sm"}
                hover:border-red-600 hover:shadow-lg hover:-translate-y-1 bg-white`}
            >
              {role.icon}
      ...
      <div className="w-full max-w-3xl mb-10">
        <h2 className="text-2xl font-semibold text-gray-800 mb-4 text-center">Job Description</h2>
        <textarea
          value={jobDesc}
      ...
      <div className="w-full max-w-3xl flex justify-end">
        <button onClick={handleNext} className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-xl shadow-md transition">
          Next →
        </button>
      </div>
    </div>
